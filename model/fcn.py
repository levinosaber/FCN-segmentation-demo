...
class IntermediateLayerGetter(nn.ModuleDict):
    ...
    twice in the forward if you want this to work.    不太明白

    Additionally, it is only able to query submodules that are directly
    assigned to the model. So if `model` is passed, `model.feature1` can
    be returned, but not `model.feature1.layer2`.     不太明白
    ...
    def __init__(self, model: nn.Module, return_layers: Dict[str, str]) -> None:
        ...
        # 调用IntermediateLayerGetter的父类的构造函数并传入layers作为参数, nn.ModuleDict的构造函数的参数可以是一个字典{str: nn.Module} 也可以是一个类似结构的迭代器
...
def fcn_resnet50(aux, num_classes=21, pretrained_backbone=False):
    ...
    if aux:
        aux_classifier = FCNHead(aux_in_channels, num_classes)
